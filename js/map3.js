	...
	svg.selectAll(".estado")
	  ...
	  populacaoScale
      	.domain([10, 50, 100])
      .range([d3.rgb(102, 51, 21, 0.6), d3.rgb(102, 51, 21, 0.8), d3.rgb(102, 51, 21, 1)]);
      ...
      .enter().append("path")
        ...
        for (var i=0; i < csv.length; i++) {
            var municipio = csv[i];
      		if (municipio.ID) {
                if (municipio.indice_atendimento_urbano_esgoto !== "NA") {
                svg.select("#demanda-"+municipio.ID)
        		.attr("fill", populacaoScale(+municipio.indice_atendimento_urbano_esgoto))
                } else {
                svg.select("#demanda-"+municipio.ID)
        		.attr("fill", "white")
                }
